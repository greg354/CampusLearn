    ...
    public class LoginController : Controller
    {
        ...
        public async Task<IActionResult> Index(LoginViewModel model)
        {
            ...
            try
            {
                ...
                if (email.EndsWith("@tutor.belgiumcampus.ac.za"))
        ...
        private async Task<IActionResult> AuthenticateStudent(string email, string hashedPassword, LoginViewModel model)
        {
            ...
            if (student == null || student.PasswordHash != hashedPassword)
            {
                ...
                _logger.LogWarning("Failed login attempt - student not found or wrong password: {Email}", email);
                return View("~/Views/Account/Login.cshtml");
            }
        ...
        {
            var tutor = await _context.Tutors.FirstOrDefaultAsync(t => t.Email.ToLower() == email);

            if (tutor == null || tutor.PasswordHash != hashedPassword)
            {
                ModelState.AddModelError("LoginError", "Invalid email or password.");
                _logger.LogWarning("Failed login attempt - tutor not found or wrong password: {Email}", email);
                return View("~/Views/Account/Login.cshtml");
            }
